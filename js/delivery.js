...
(function startWeightPoll() {
  function poll() {
    function displayWeightError() {
      var weightEl = document.getElementById('deliveryWeight');
      if (weightEl) {
        weightEl.textContent = '— kg';
        weightEl.style.color = '#9ca3af';
      }
      var barEl = document.getElementById('deliveryWeightBar');
      if (barEl) {
        barEl.style.width = '0';
      ...
      var topbarW = document.getElementById('topbar-weight-val');
      if (topbarW) {
        topbarW.textContent = 'ERR';
        topbarW.style.color = '#ef4444';
      }
      var badge = document.getElementById('del-weight-badge');
      if (badge) badge.textContent = '— kg';
    }

    console.debug('[delivery] polling', SERVER_URL + '/weight/status');
    fetch(SERVER_URL + '/weight/status')
      .then(function(r) { console.debug('[delivery] /weight/status HTTP', r.status); if (r.ok) return r.json(); return null; })
      .then(function(d) {
        console.debug('[delivery] /weight/status json', d);
        if (!d && d !== 0) return;
        /* Suppress noisy weight for 3 s after robot start */
        if (typeof _weightSuppressUntil !== 'undefined' && Date.now() < _weightSuppressUntil) {
          d = { weight: 0, threshold: d && d.threshold };
        }
        var kg        = ((d && typeof d.weight === 'number') ? d.weight : 0).toFixed(2);
        var threshold = 2.0;
        ...
        if (weightEl) {
          weightEl.textContent = kg + ' kg';
          weightEl.style.color = (d && d.weight >= threshold) ? '#10b981' : '#9ca3af';
        }
        /* Weight bar */
        var barEl = document.getElementById('deliveryWeightBar');
        if (barEl) {
          var pct = Math.min(100, (((d && d.weight) || 0) / threshold) * 100);
          barEl.style.width      = pct + '%';
          barEl.style.background = (d && d.weight >= threshold) ? '#10b981' : '#3b82f6';
        }
        /* Topbar weight */
        var topbarW = document.getElementById('topbar-weight-val');
        if (topbarW) {
          topbarW.textContent = kg + ' kg';
          topbarW.style.color = (d && d.weight >= threshold) ? '#10b981' : 'var(--accent)';
        }
        /* Bottom bar badge */
        var badge = document.getElementById('del-weight-badge');
        if (badge) badge.textContent = kg + ' kg';
      })
      .catch(function(err) { console.error('[delivery] Weight status fetch failed', err); });
  }
  poll();
